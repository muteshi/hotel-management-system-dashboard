...
import logo from "../../../assets/images/web-guru-png.png";
...
const Sidebar = (props) => {
  ...
  const setNavActive = (item) => {
    ...
    setMainMenu({ mainmenu: NEWMENUITEMS });
  };

  // Click Toggle menu
  const toggletNavActive = (item) => {
    if (!item.active) {
      NEWMENUITEMS.forEach((a) => {
        if (NEWMENUITEMS.includes(item)) a.active = false;
