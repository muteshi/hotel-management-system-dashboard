...
import { getConferenceHotels } from "../../services/hotelService";
...
class ConferenceBookingForm extends Form {
  state = {
    data: {
      checkin: "",
      checkout: "",
  ...
  async populateHotels() {
    const { data: conferences } = await getConferenceHotels();
  ...
  doSubmit = async () => {
    try {
      ...
      await saveBooking(conferenceBookingData);
  ...
  render() {
    ...
    let roomQtyOptions = _.range(0, roomData.total_Rooms + 1).map(function (
      ...
      const confrenceRoomPrice =
        roomData.room_Price * roomData.max_adults * num;
      const conferencerRoomPriceShow = `${num} rooms for ${
        roomData.max_adults * num
      } guests at (KES  ${confrenceRoomPrice})`;

      return {
        value: num,
        label: `${confrenceRoomPrice === 0 ? 0 : conferencerRoomPriceShow}`,
      };
    });

    let roomGuestsOptions = _.range(0, roomData.max_adults + 1).map(function (
      num
    ) {
      return {
        value: num,
    ...
    let conferenceHotelOptions = conferences.map(function (hotel) {
      return {
        value: hotel.id,
        label: hotel.name,
    ...
    let conferenceRoomOptions = rooms
      .filter((room) => {
            ...
            <div className="jumbotron">
              <form onSubmit={this.handleSubmit} style={{ marginTop: -10 }}>
                ...
                <Select
                  ...
                  options={conferenceHotelOptions}
                ...
                <div className="row" style={{ marginTop: 22 }}>
                  ...
                  <div className="col-sm-6 col-md-6">
                    ...
                    <Select
                      ...
                      className="mb-3"
                      name="form-field-name"
                      onChange={this.handleGuestsTotalSelectChange}
                      options={roomGuestsOptions}
